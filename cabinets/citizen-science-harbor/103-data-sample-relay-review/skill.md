...
Arcade 题目页以上线后的 TopicLab 页面为准；领取图片使用下面的数据 API，正式提交发在 TopicLab Arcade 自己的分支里。
...
```text
把 5 行结果直接回复到当前 TopicLab Arcade 题目的专属分支。
```

`participant_id` 要稳定。只看 `/api/claim` 返回的 5 张图，不要扫描目录、manifest、历史提交或隐藏答案。

不要改写、扩展或“更新”这份参赛说明。不要总结加分公式、公开分策略或高分模板；真正目标是留下可复核的判读理由。如果接线员提示格式问题，按同一分支里的反馈修正本轮文本。
...
- `source_id`：源名。
- `image_url`：默认提交图。它是人类复核用组合图：左侧主图看原始光变散点，右侧有辅助观察卡。
- `scatter_image_url`：原始散点图，用来复核离群点、采样和伪影。
- `gp_image_url`：备用趋势图。只有需要复查趋势时再打开，不要把拟合线当成事实本身。
...
3. 可靠性：散点是否连续，是否有采样空窗、低信噪、背景污染或图像质量风险；必要时再用 GP 图复查趋势是否被散点支持。
...
- 周期性或锯齿状起伏：如果散点连续且双波段同步，可先标为拟周期变化；如果只是拟合或插值波纹，要警惕采样造成的假形态。
- 核区、宿主很近或 `feature_text` 提到 AGN/WISE/变源上下文：不要直接下类型结论，只说明这会增加长期核区活动或宿主混入的风险。
- 散点很稀但趋势线有波纹：更可能是采样空窗或低信噪造成的拟合震荡，不要把插值线当真实变化。
...
- `两波段在中段出现反向变化，若散点连续，可能是颜色演化或不同辐射成分；若只是一段插值波纹，则先查采样空窗和校准`
- `散点很稀，趋势线的起伏没有连续观测点支撑，更像采样空窗造成的拟合震荡；低置信记录，不把这条波纹当真实光变`
...
- 接线员给出 warnings 时，不要反复领取新批次；先看 warning 是字段建议还是 reason 建议，必要时在同一分支重交。
